...
public class UploadImageAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			iv_image.setPadding(10,10,0,0);
			iv_image.setLayoutParams(new AbsListView.LayoutParams(ImageUtils.getWidth(context) / 6 - 5,
					ImageUtils.getWidth(context) / 6 - 5) );
		...
		if(getItem(position) == null ){
			iv_image.setImageResource(R.mipmap.defult_add);

		}else{
			// 获取图片缩略图，避免OOM
			Bitmap bitmap = ImageUtils.getImageThumbnail((String)getItem(position),
					ImageUtils.getWidth(context) / 6 - 5, ImageUtils.getWidth(context) / 6 - 5);
